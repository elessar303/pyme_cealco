var eventos_form = {
    ...
    Limpiar: function() {
        //$("#cantidadunitaria, #porcentaje_ganancia, #precio, #items, #precio_unitario, #precio_sugerido, #pvp, #operatividad, #margen_ganancia, #costo_operativo, #costo_iva, #codigoBarra, #almacen, #descripcionitem, #codigofabricante, #cantidadunitaria, #costounitario, #totalitem_tmp, #cantidaddeberia, #observacion, #nlote").val("");
        $("#precio_unitario, #porcentaje_operatividad, #operatividad, #costo_sin_iva, #costo_iva, #precio_sugerido, #porcentaje_ganancia, #margen_ganancia, #pvp").val("");
        if ($("#rubros").val()!=0 ||
            $("#precio_sugerido").val()=="" ||
    ...
    IncluirRegistros: function(options) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                campos += $.inputHidden("_costo_iva", options.costo_iva, "[]");
