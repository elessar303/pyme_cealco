$(document).ready(function() {
    ...
    $('tr.detalle').click(function() {
        ...
        estatus = objeto.find("input[name='estatus']").val();
        id_cliente = objeto.find("input[name='id_cliente']").val();
        id_tipo_movimiento_almacen = objeto.find("input[name='id_tipo_movimiento_almacen']").val();
        //Cargamos los debitos y creditos
        $.ajax({
            type: 'GET',
            data: 'opt=EstadoCuenta&id_transaccion=' + id_transaccion + '&id_tipo_movimiento_almacen=' + id_tipo_movimiento_almacen + '&estatus=' + estatus + '&id_cliente=' + id_cliente,
            url: '../../libs/php/ajax/ajax.php',
            beforeSend: function() {
            },
            success: function(data) {
