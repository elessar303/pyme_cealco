...
$cliente = new Clientes();
mysql_set_charset('utf8');
$campos = $cliente->ObtenerFilasBySqlSelect("SELECT * FROM $bdSelectrapyme.unidades order by descripcion");
foreach ($campos as $key => $item) {
    $arraySelectOption2[] = $item["cod_unidad"];
    $arraySelectOutPut2[] = utf8_encode($item["descripcion"]);
}
...
//Territorio
$campos = $cliente->ObtenerFilasBySqlSelect("SELECT distinct(territorio) FROM $bdCentral.votacion order by territorio");
foreach ($campos as $key => $item) {
    $arraySelectOption7[] = utf8_encode($item["territorio"]);
    $arraySelectOutPut7[] = utf8_encode($item["territorio"]);
}
$smarty->assign("option_values_tipo_territorio", $arraySelectOption7);
$smarty->assign("option_output_tipo_territorio", $arraySelectOutPut7);
