...
//nuevo
$campos = $provee->ObtenerFilasBySqlSelect("SELECT * FROM ".DB_SELECTRA_PYMEPP.".estados;");
foreach ($campos as $key => $item) {
    $arraySelectOption_estado[] = $item["codigo_estado"];
    $arraySelectoutPut_estado[] = $item["nombre_estado"];
}
$smarty->assign("option_values_estado", $arraySelectOption_estado);
$smarty->assign("option_output_estado", $arraySelectoutPut_estado);

//rubros
//nuevo
$campos = $provee->ObtenerFilasBySqlSelect("SELECT cod_grupo, descripcion FROM grupo;");
foreach ($campos as $key => $item) {
    $arraySelectOption_rubro[] = $item["cod_grupo"];
    $arraySelectoutPut_rubro[] = $item["descripcion"];
}

$smarty->assign("option_values_rubro", $arraySelectOption_rubro);
$smarty->assign("option_output_rubro", $arraySelectoutPut_rubro);
