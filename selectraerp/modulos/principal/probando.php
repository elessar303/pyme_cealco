...
while ($archivo = $directorio->read()) {
    ...
    if (substr($archivo,0,1)!=".") {
        ...
        while($filas[$i]!=NULL) {
            ...
            $comprobar=$comunes->ObtenerFilasBySqlSelect("select id from $pymeC.puntos_venta where codigo_siga__punto='000".$values[61]."'");
            ...
            $sql ="INSERT INTO $pymeC.ventas(id_tickets, TICKETTYPE, TICKETID, PERSON, CUSTOMER, STATUS, TICKET, LINE, PRODUCT, 
		        ...
		        //if(is_numeric($values[61])){ //Validamos que el codigo SIGA del punto sea un valor numerico
			    ...
			    error_log("Error al insertar fila del archivo ".$archivo." Codigo SIGA: ".$values[61]." dentro del archivo".("\r\n"), 3, "/var/www/pyme/selectraerp/uploads/error/error.log");
                 ...
                 error_log("Error al verificar archivo ".$archivo." Codigo SIGA: ".$values[61]."  fecha o codigo siga dentro del archivo".("\r\n"), 3, "/var/www/pyme/selectraerp/uploads/error/error.log");
             }
...
and codigo_siga in (SELECT  codigo_siga_punto
FROM  $pymeC.puntos_venta 
WHERE  tipo_sincro =2)");
...
AND c.codigo_estado = d.codigo_estado_punto
AND codigo_siga = d.codigo_siga_punto
and codigo_siga not in (SELECT  codigo_siga_punto
FROM  $pymeC.puntos_venta 
WHERE  tipo_sincro =2)");
}
