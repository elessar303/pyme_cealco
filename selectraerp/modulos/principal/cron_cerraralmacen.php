...
$money=$almacen->ObtenerFilasBySqlSelect("select money from closedcash_pyme where serial_caja='".impresora_serial."' and fecha_fin is null order by secuencia desc limit 1");
    ...
    $sql="select b.id_cliente from 
        kardex_almacen as a 
        inner join clientes as b on a.id_cliente=b.id_cliente 
        inner join kardex_almacen_detalle as c on a.id_transaccion=c.id_transaccion
        where a.facturado=0 
        and a.nro_factura is not null 
    ...
    foreach ($despachos as $key => $arraycliente) 
    {
        ...
        $sql="SELECT *,kardex_almacen.estado as estatus from kardex_almacen, clientes, kardex_almacen_detalle  
        WHERE kardex_almacen.id_cliente=clientes.id_cliente 
        and kardex_almacen.id_transaccion=kardex_almacen_detalle.id_transaccion 
        and kardex_almacen_detalle.id_almacen_entrada='".$_GET['almacen']."'
        and kardex_almacen.facturado=0 
        and kardex_almacen.nro_factura is not null 
        and kardex_almacen.nro_factura<>'' and clientes.id_cliente=".$despacho."
        group by kardex_almacen.nro_factura";
        ...
        "
            ...
            inner join ubicacion c on b.id_ubi_entrada=c.id where c.ocupado=1 and id_cliente='".$despacho."' and b.id_almacen_entrada='".$_GET['almacen']."'
        ";
            ...
            foreach ($ubicaciones as $key => $value3)
            {
                ...
                if($buscarserviciosubicacion!=NULL)
                {
                    ...
                    $sql="select a.peso, a.costo_declarado 
                        ...
                        where b.id_cliente='".$despacho."' and a.id_almacen_entrada='".$_GET['almacen']."' and a.id_ubi_entrada='".$value3['ubicacion']."' and c.ocupado=1 and b.facturado=0";
