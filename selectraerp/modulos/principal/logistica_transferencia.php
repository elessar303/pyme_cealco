...
$ubica=$ubicacion->ObtenerFilasBySqlSelect("SELECT nombre_estado as nombre, codigo_estado as id  from ".DB_SELECTRA_PYMEPP.".estados");
...
// si el usuario hizo post
    if (!$pendiente) 
    {
        ...
        INSERT INTO pda_maestro(tipo,  orden_compra, observacion, estatus, usuario_logistica, transporte) 
        VALUES (
            '".$_POST['tipo_log']."' , '{$_POST["nro_documento"]}',
            '{$_POST["observaciones"]}',  
            0, '{$login->getUsuario()}', '{$_POST["transporte"]}');";
        ...
        for ($i = 0; $i < (int) $_POST["input_cantidad_items"]; $i++) 
        {
            ...
            $kardex_almacen_detalle_instruccion = "INSERT INTO pda_detalle(id_pda_maestro, id_producto, cantida_x_kilo, id_instalacion_origen, id_instalacion_destino, fecha_inicio, fecha_fin, observaciones) 
                VALUES (
                    '{$id_transaccion}','{$_POST["_id_item"][$i]}','{$_POST["_cantidad"][$i]}', '{$_POST["punto_origen"][$i]}','{$_POST["punto_destino"][$i]}', '".$fecha_planificacion."', '".$fecha_planificacion_fin."','{$_POST["observacion_detalle"][$i]}' );";
