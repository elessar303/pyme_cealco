...
mysql_set_charset('utf8');
$punto = $cliente->ObtenerFilasBySqlSelect("SELECT gerencia FROM $bdCentral.votacion GROUP BY gerencia ORDER BY gerencia");
foreach ($punto as $key => $puntos) {
    $arraySelectOption[] = $puntos["gerencia"];
    $arraySelectOutPut1[] = $puntos["gerencia"];
}
...
$cliente = new Clientes();
mysql_set_charset('utf8');
$campos = $cliente->ObtenerFilasBySqlSelect("SELECT estado_centro FROM $bdCentral.votacion GROUP BY estado_centro ORDER BY estado_centro");
foreach ($campos as $key => $item) {
    $arraySelectOption2[] = $item["estado_centro"];
    $arraySelectOutPut2[] = $item["estado_centro"];
}
