...
$boton_form='NO';
$smarty->assign("boton_form", $boton_form);
...
if (isset($_POST['buscar']) || $tipob != NULL) {
    ...
    if($_SESSION['tipo_departamento']!=52){
    switch ($tipob) {
        case "exacta":
        ...
        switch ($tipob) {
        case "exacta":
            echo $instruccion = $comunes->buscar_exacta($tabla, $des, $busqueda,"AND unidades.descripcion=votacion.gerencia group by cedula_emp",$columnas);
            break;
        case "todas":
            $instruccion = $comunes->buscar_todas($tabla, $des, $busqueda,"AND unidades.descripcion=votacion.gerencia group by cedula_emp",$columnas);
            break;
        case "cualquiera":
            $instruccion = $comunes->buscar_cualquiera($tabla, $des, $busqueda,"AND unidades.descripcion=votacion.gerencia group by cedula_emp",$columnas);
            break;
        }
    }
}else {
    ...
    $instruccion = "SELECT * FROM $tabla where unidades.descripcion=votacion.gerencia group by cedula_emp"; 
    }
}
...
$tabla_1="selectrapyme_central.votacion";
$sql="UPDATE $tabla_1 set hora_vota='2017-07-30 ".$hora.":".$min.":00' WHERE votacion.id=".$id.""; 
