...
while ($archivo = $directorio->read()) {
    ...
    $fecha_ins = date_format( new DateTime($values[55]), 'Y-m-d H:i:s' );

    $anno_1 = substr($fecha_ins, 2, 2);
    $mes_1 = substr($fecha_ins, 5, 2);
    $ventas_mes_anno="ventas_".$mes_1."_".$anno_1; 
		
    if($i==0){
    $sql_val="SELECT TICKET, codigo_siga, DATENEW FROM $pymeC.$ventas_mes_anno WHERE TICKET='".$values[6]."' AND codigo_siga='000".$values[61]."'"; 
    $validar=$comunes->ObtenerFilasBySqlSelect($sql_val);
    $num_verificar=$comunes->getFilas($validar);
    }

    if($num_verificar!=0 and $i==0){
    echo" <script type='text/javascript'>
          alert('Este archivo ya fue Subido!');
          history.go(-1);    
          </script>";
