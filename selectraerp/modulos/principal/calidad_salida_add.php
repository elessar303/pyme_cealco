...
$punto = $almacen->ObtenerFilasBySqlSelect("SELECT * from roles_firma where descripcion_rol=5");
...
$punto = $almacen->ObtenerFilasBySqlSelect("SELECT * from roles_firma where descripcion_rol=2");
foreach ($punto as $key => $puntos) {
...
$punto = $almacen->ObtenerFilasBySqlSelect("SELECT * from roles_firma where descripcion_rol=4");
foreach ($punto as $key => $puntos) {
...
// si el usuario hizo post
    ...
    # Verificar que no se trata de una compra con estatus de entrega de productos "Pendiente"
        ...
        $kardex_almacen_instruccion = "INSERT INTO calidad_almacen (
            ...
            `fecha_creacion`, `estado`, `fecha_ejecucion`, `id_documento`, `empresa_transporte`, `id_conductor`, `placa`, `guia_sunagro`, `orden_despacho`, `almacen_procedencia`, `id_proveedor`,`id_ticket_entrada`, `prescintos`, `tipo_acta`)
        VALUES (
            NULL , '4', '{$_POST["autorizado_por"]}',
            '{$_POST["observaciones"]}', '{$_POST["input_fechacompra"]}', '{$login->getUsuario()}', 
            CURRENT_TIMESTAMP, 'Entregado', CURRENT_TIMESTAMP, '{$_POST["nro_documento"]}', '{$_POST["empresa_transporte"]}', '{$id_conductor[0]["id_conductor"]}', '{$_POST["placa"]}', '{$_POST["codigo_sica"]}', '{$_POST["orden_despacho"]}', '{$_POST["puntodeventa"]}', '{$_POST["id_proveedor"]}','{$_POST["id_ticket"]}', '{$_POST["prescintos"]}', 2);";
