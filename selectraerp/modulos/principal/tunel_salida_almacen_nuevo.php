...
$sql="select id_tipo_movimiento_almacen from tipo_movimiento_almacen where descripcion='Descargo Tunel'";
...
{ // si el usuario iso post
    ...
    $kardex_almacen_instruccion = "
        ...
        VALUES (
        NULL, '14', '" . $_POST["autorizado_por"] . "', '" . $_POST["observaciones"] . "',
    ...
    for ($i = 0; $i < (int) $_POST["input_cantidad_items"]; $i++) 
    {
        ...
        if (count($campos) > 0) 
        {
            ...
            //tipo cliente
            $sql="select cod_tipo_cliente from clientes where id_cliente={$_POST['cliente']}";
