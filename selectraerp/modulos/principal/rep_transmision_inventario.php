...
$punto = $cliente->ObtenerFilasBySqlSelect("SELECT `nombre_punto`,codigo_siga_punto as siga
    from selectrapyme_central.puntos_venta
    where estatus='A'");
...
/*QUERY ORIGINAL de JA
$sql="SELECT C.nombre_estado,A.codigo_siga,B.nombre_punto, B.tipo_sincro, A.fecha, A.fecha as color, reporto
FROM selectrapyme_central.reportes_inventario A, selectrapyme_central.puntos_venta B, selectrapyme_central.estados C
...
/*MODIFICADO Para mostrar los centros de distribucion HZ*/
$sql="SELECT C.nombre_estado,A.codigo_siga,B.nombre_punto, B.tipo_sincro, A.fecha, A.fecha as color, reporto, B.version_pyme
FROM selectrapyme_central.reportes_inventario A, selectrapyme_central.puntos_venta B, selectrapyme_central.estados C
WHERE A.codigo_siga=B.codigo_siga_punto
AND B.codigo_estado_punto=C.codigo_estado
AND B.estatus='A'
...
$since_start->d.' days<br>';
    ...
    if ( $since_start->d==0) {
    	...
    	if($since_start->d>2 &&  $since_start->d<2){
    		return 'rojo';
