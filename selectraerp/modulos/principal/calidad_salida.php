...
if (isset($_POST['buscar']) || $tipob != NULL) {
    ...
    $instruccion = $instruccion . " and tipo_acta=2 order by fecha_creacion desc";
    //exit(0);
} else {
    $instruccion = "SELECT * FROM $tabla as k 
    LEFT JOIN tickets_entrada_salida AS tick ON k.id_ticket_entrada = tick.id
    LEFT JOIN transporte_conductores AS c ON tick.id_conductor = c.id
    LEFT JOIN clientes AS cli ON k.id_proveedor = cli.id_cliente   where tipo_acta=2  order by fecha_creacion desc";
}
...
//**************************************************************************
$smarty->assign("option_values", array("nombre","cod_acta_calidad", "observacion", "almacen_procedencia"));
