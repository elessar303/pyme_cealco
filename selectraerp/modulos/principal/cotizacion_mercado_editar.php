...
# Objetivos:
# Editar los productos a cotizas
# Observaciones:
...
//cargando productos de la citización hecha
if ($_GET["cod"] != "") {
    $sql_products = "SELECT i.codigo_barras,i.descripcion1,cmd.costo_sin_iva,cmd._ivaproduct,
    cmd._precio_sugerido,cmd._margen_ganancia,cmd._pvp,cmd._estatus_producto,cme.estatus_name
    FROM selectrapyme_central.cotizacion_mercado_detalle cmd
    INNER JOIN selectrapyme_central.cotizacion_mercado cm ON cm.id_estudio = cmd.id_estudio
    ...
    $productos_editar = $almacen->ObtenerFilasBySqlSelect($sql_products);
    $smarty->assign("productos_editar",$productos_editar);
    $smarty->assign("id_estudio", $_GET["cod"]);
...
// si el usuario hizo post
    ...
    for ($i = 0; $i < (int) $_POST["input_cantidad_items"]; $i++) {
        ...
        $floativaproduct = floatval($ivaproduct[$i]);
        $floatmargenganancia = floatval($margenganancia[$i]);
        $sumapvp = ($floativaproduct + $floatmargenganancia);
        ...
        $ingreso_detalle_estudio = "INSERT INTO $pyme.cotizacion_mercado_detalle (
            `_precio_sugerido`,`id_estudio`, `id_producto`, `costo_sin_iva`, `_ivaproduct`,`_margen_ganancia`,`_pvp`,
            `_precio_unitario`,`_porcentaje_operatividad`,`_operatividad`,`_porcentaje_ganancia`, `_estatus_producto`,`fec_ingre_mercadeo`,`_troquel`)
            VALUES('{$precio_sug[$i]}','".$_GET["cod"]."', '{$productos[$i]}', '{$precios[$i]}', '{$ivaproduct[$i]}',
                '{$margenganancia[$i]}', '{$sumapvp}', '{$preciounitario[$i]}', '{$porcentajeoperatividad[$i]}',
                '{$operatividad[$i]}', '{$porcentajeganancia[$i]}', '3', now(), '{$troquel[$i]}');
            ";
