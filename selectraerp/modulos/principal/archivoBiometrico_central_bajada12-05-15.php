...
$fechaICon=strftime( "%Y-%m-%d 00:00:00",strtotime("-7 day"));
...
if($zip->open($path_consolidados.'/'.$nombrezip,ZIPARCHIVE::CREATE)===true){
        $zip->addFile($path_consolidados.'/'.$archivo_consolidado,$archivo_consolidado);
//$nombre_final=  substr($archivo_consolidado, 0,-4);
//    $zip->addFile($archivo_consolidado);
    $zip->close();
    chmod($path_consolidados.'/'.$nombrezip,  0777);
    //echo 'descomprimio';
	} else {
