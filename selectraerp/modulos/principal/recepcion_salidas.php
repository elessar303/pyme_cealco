...
{   
    ...
    $smarty->assign("fecha_hasta", $fecha_hasta);
}
if (isset($_POST['buscar']) || $tipob != NULL || isset($_POST['aceptar'])) 
{
    ...
    $smarty->assign("fecha_hasta", $_POST['fecha2']);
    ...
    $smarty->assign("despacho", $despacho);
    ...
    $instruccion=" select a.cod_movimiento, a.fecha, a.id_documento, a.rif, a.guia_sunagro, a.nombre_proveedor, pv.nombre_punto, a.tipo_movimiento, a.observacion_cabecera,a.almacen_salida, a.ubicacion_salida, d.descripcion as tipo_despacho, sum(a.cantidad) as total  from ".DB_SELECTRA_PYMEPP.".kardex_13_17_s a, ".DB_SELECTRA_PYMEPP.".puntos_venta pv, ".DB_SELECTRA_PYMEPP.".estados as c, tipo_despacho as d
        where pv.codigo_estado_punto=c.codigo_estado and a.id_tipo_despacho=d.id and a.codigo_siga=SUBSTRING(pv.codigo_siga_punto, 4) and fecha between '".$_POST['fecha']."' and '".$_POST['fecha2']."' 
        ".$estados.$punto.$doc_siscol.$documento.$rif.$guia.$nombre_proveedor.$descripcion.$codigo_barras.$despacho. " and (a.tipo_movimiento='Descargo' or a.tipo_movimiento='Pedido') group by a.cod_movimiento order by fecha, cod_movimiento desc";
...
//Tipos Despacho
$ubica=$comunes->ObtenerFilasBySqlSelect("select * from  tipo_despacho");

foreach ($ubica as $key => $item) {
    $desi[] = $item["descripcion"];
    $vali[] = $item["id"];
}
$smarty->assign("option_values_des_despacho", $desi);
$smarty->assign("option_values_id_despacho", $vali);

$smarty->assign("registros", $campos);
$smarty->assign("instruccion", $instruccion);
$smarty->assign("cabecera", array("Doc. Siscol", "Fecha", "Descripci&oacute;n", "Almacen Salida","Ubicacion Salida", "Tipo de Despacho", "Unidades"));
...
//**************************************************************************
$smarty->assign("option_values", array("cod_movimiento", "fecha", "id_documento", "nombre_punto"));
$smarty->assign("option_output", array("Doc. Siscol", "Fecha", "Documento", "Almacen"));
